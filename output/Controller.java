...
{ for (int i = 0; i < patientgps.size(); i++)
  ...
  {   System.out.println("" + ( "Patient " + this + " has no Id value" ));
}
  }

    public void check2(PatientGP p)
  {   if (!(p.getId().equals(this.getId())) && p.getname1().equals(this.getname1()) && p.getname2().equals(this.getname2()) && p.getdob() == this.getdob()) 
  {   System.out.println("" + ( "Duplicate patients?: " + this + " and " + p ));
}
  }

    public void check2outer()
  ...
  { PatientGP p = (PatientGP) _range1.get(_i0);
    ...
    public void check3()
  {   if (Referral.getAllreferredTo(this.getreferrals()).contains("optician")) 
  {   System.out.println("" + ( "Patient: " + this + " was referred to an optician" ));
}
  }

    public void migrategp1()
...
public void setpostcode(Location locationx, String postcode_x) 
  ...
  public void check2(PatientGP patientgpx,PatientGP p)
  {   patientgpx.check2(p);
   }

  public void check2outer(PatientGP patientgpx)
  {   patientgpx.check2outer();
   }
  ...
  public void migrategp1(PatientGP patientgpx)
  {   patientgpx.migrategp1();
   }
  ...
  for (int patientgpcheck1x_ind2 = 0; patientgpcheck1x_ind2 < patientgpcheck1x.size(); patientgpcheck1x_ind2++)
  { Controller.inst().check1((PatientGP) patientgpcheck1x.get(patientgpcheck1x_ind2)); }

       List patientgpcheck2outerx = new Vector();
  patientgpcheck2outerx.addAll(Controller.inst().patientgps);
  for (int patientgpcheck2outerx_ind3 = 0; patientgpcheck2outerx_ind3 < patientgpcheck2outerx.size(); patientgpcheck2outerx_ind3++)
  { Controller.inst().check2outer((PatientGP) patientgpcheck2outerx.get(patientgpcheck2outerx_ind3)); }

       List patientgpcheck3x = new Vector();
  patientgpcheck3x.addAll(Controller.inst().patientgps);
  for (int patientgpcheck3x_ind4 = 0; patientgpcheck3x_ind4 < patientgpcheck3x.size(); patientgpcheck3x_ind4++)
  { Controller.inst().check3((PatientGP) patientgpcheck3x.get(patientgpcheck3x_ind4)); }
    ...
    public void migrateGP() 
  { 

       List patientgpmigrategp1x = new Vector();
  patientgpmigrategp1x.addAll(Controller.inst().patientgps);
  for (int patientgpmigrategp1x_ind5 = 0; patientgpmigrategp1x_ind5 < patientgpmigrategp1x.size(); patientgpmigrategp1x_ind5++)
  { Controller.inst().migrategp1((PatientGP) patientgpmigrategp1x.get(patientgpmigrategp1x_ind5)); }
